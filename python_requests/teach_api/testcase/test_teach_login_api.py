...
class LoginApi(unittest.TestCase):
    ...
    def test_teach_login_api(self,data,login_testcase_retcode,logintest_num,logintest_title):
        #把测试标题写入到测试报告中(self._testMethodDoc,self._testMethodDoc 固定方法，把标题和编号写入到测试报告中)
        self._testMethodDoc = logintest_title
        #把测试编号写入到测试报告中
        self._testMethodName = logintest_num
        #响应结果
        res = Teach_Lesson_Manage_Api().teach_login_api(data)
        #获取响应结果中的retcode的值
        res_retcode = res["retcode"]
        #获取接口测试用例中的retcode的值
        testcase_retcode = (login_testcase_retcode["retcode"])
